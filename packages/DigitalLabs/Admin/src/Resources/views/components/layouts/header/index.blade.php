...
<header class="sticky top-0 z-[10001] flex items-center justify-between border-b bg-white px-2 py-2 dark:border-gray-800 dark:bg-gray-900 sm:px-4 sm:py-2.5">
    <div class="flex items-center gap-1 sm:gap-1.5">
        ...
        <a href="{{ route('admin.dashboard.index') }}" class="flex-shrink-0">
            ...
            @else
                <img
                    src="{{ request()->cookie('dark_mode') ? qubix_asset('images/dark-logo.svg') : qubix_asset('images/logo.svg') }}"
                    class="h-8 w-auto sm:h-10"
                    id="logo-image"
                    alt="{{ config('app.name') }}"
                />
                                ...
                                <div class="flex gap-2 sm:gap-2.5">
                                    ...
                                    >
                                        <template v-if="! product.images.length">
                                            <img src="{{ qubix_asset('images/product-placeholders/front.svg') }}" class="h-full w-full object-cover">
            ...
            template: '#v-notifications-template',
                ...
                data() {
                    return {
                        ...
                        orderTypeMessages: {
                            {{ \DigitalLabs\Sales\Models\Order::STATUS_PENDING }}: "@lang('admin::app.notifications.order-status-messages.pending')",
                            {{ \DigitalLabs\Sales\Models\Order::STATUS_CANCELED }}: "@lang('admin::app.notifications.order-status-messages.canceled')",
                            {{ \DigitalLabs\Sales\Models\Order::STATUS_CLOSED }}: "@lang('admin::app.notifications.order-status-messages.closed')",
                            {{ \DigitalLabs\Sales\Models\Order::STATUS_COMPLETED }}: "@lang('admin::app.notifications.order-status-messages.completed')",
                            {{ \DigitalLabs\Sales\Models\Order::STATUS_PROCESSING }}: "@lang('admin::app.notifications.order-status-messages.processing')",
                            {{ \DigitalLabs\Sales\Models\Order::STATUS_PENDING_PAYMENT }}: "@lang('admin::app.notifications.order-status-messages.pending-payment')",
                        }
                    }
                },
